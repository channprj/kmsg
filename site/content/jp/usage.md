...
KakaoTalkのロック画面が表示されている場合は、コマンドの実行前にロックを解除します。ロックパスワードはKakaoTalk側で設定する値でアカウントのパスワードとは別物なので、最初の一度だけ入力すればアカウント認証情報と一緒に暗号化して保存します。

ロック解除はコマンドごとに一度だけ試行します。ロックパスワードを何度も間違えるとKakaoTalkがアカウントをログアウトさせるためです。パスワードが拒否された場合は保存値を破棄し、次のコマンドで再入力を求めます。

ターミナルを持たない呼び出し元（`kmsg mcp-server`、`kmsg watch`、cron）は入力を受け取れません。ターミナルで一度ロックを解除してパスワードを保存しておけば、以降は自動でロックを解除します。
