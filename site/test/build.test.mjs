...
test("English homepage is semantic, canonical, and README-derived", async () => {
  ...
  assert.match(
    html,
    /hreflang="ko" href="https:\/\/channprj\.github\.io\/kmsg\/ko\/"/,
  );
  ...
  assert.match(html, /나만의 Hermes 시스템 구축 방법/);
...
test("Korean homepage has localized metadata and visible answers", async () => {
  const html = await readOutput("ko/index.html");

  assert.match(html, /<html lang="ko"/);
  assert.match(html, /<body data-source="README\.ko\.md">/);
  assert.match(
    html,
    /<link rel="canonical" href="https:\/\/channprj\.github\.io\/kmsg\/ko\/">/,
  );
  assert.match(html, /macOS용 카카오톡 CLI 및 MCP 서버/);
  assert.match(html, /자주 묻는 질문/);
  assert.match(html, /kmsg는 무엇인가요/);
